...
for size in ${img_sizes[@]}; do
    ...
    python3 export.py --weights ./yolov7-tiny.pt --grid --end2end --simplify --topk-all 100 --iou-thres 0.65 --conf-thres 0.35 --img-size ${hw_array[0]} ${hw_array[1]} --max-wh ${hw_array[1]} && mv ./yolov7-tiny.onnx /models/yolov7-tiny-${size}.onnx

    onnx2tf -i /models/yolov7-tiny-${size}.onnx -o /models/tflite_yolov7_tiny_${size} --output_signaturedefs
